...
# 尝试导入 datasets 库，用于加载 Hugging Face 数据
try:
...
if __name__ == "__main__":
    ...
    # Mind2Web 参数 (新增)
    parser.add_argument('--mind2web', action='store_true',
                        help='Load Mind2Web dataset directly from Hugging Face')
    parser.add_argument('--split', type=str, default='train',
                        help='Dataset split for Mind2Web (train, test_task, etc.)')
    ...
    # 模式 1: 处理 Mind2Web (Hugging Face)
    if args.mind2web:
        if load_dataset is None:
            print("错误: 未检测到 datasets 库。请运行 `pip install datasets`。")
            exit(1)
            
        print(f"正在加载 osunlp/Mind2Web 数据集 ({args.split})...")
        try:
            # 加载数据集
            dataset = load_dataset("osunlp/Mind2Web", split=args.split)
            
            # 遍历转换数据格式
            # Mind2Web 结构: confirmed_task (问题), action_reprs (动作/答案), url (可能在不同层级)
            for item in dataset:
                # 获取问题
                q = item.get('confirmed_task', "")
                
                # 获取答案 (这里将 action_reprs 列表转为字符串作为参考答案)
                ans = str(item.get('action_reprs', ""))
                
                # 获取 URL
                if args.force_url and args.start_url:
                    task_url = args.start_url
                else:
                    # Mind2Web 通常包含 url 字段，或者需要从 annotation 中提取
                    # 这里尝试直接获取 'url'，如果不存在则回退
                    task_url = item.get('url') or args.start_url
            ...
            print(f"从 Mind2Web 加载了 {len(tasks_to_process)} 个任务")
            
        except Exception as e:
            print(f"加载 Mind2Web 失败: {e}")
            exit(1)
    ...
    # 模式 3: 处理 Parquet 文件
        ...
        print("错误: 必须提供数据源参数 (--mind2web, --jsonl_path, 或 --data_path)")
        exit(1)
